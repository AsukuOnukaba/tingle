...
import { Heart, Mail, Lock, Eye, EyeOff, User, Wallet } from "lucide-react";
      ...
      if (isLogin) {
        ...
        if (success) {
          ...
          if (session) {
            ...
            const roles = rolesData?.map(r => r.role) || [];
            navigate(roles.includes('admin') ? "/admin" : "/home", { replace: true });
          }
        } else {
          // Wallet not registered, switch to sign up
          toast({
        ...
        // Sign up with wallet
        const displayName = formData.username || basename || `User ${walletAddress.substring(0, 6)}`;
        const success = await signUpWithWallet(displayName);
        if (success) {
          navigate("/home", { replace: true });
        }
      }
              ...
              <div className="mt-6">
                ...
                <div className="grid grid-cols-2 gap-3 mt-4">
                  <Button
                    type="button"
                    variant="outline"
                    className="bg-card border-border hover:bg-muted transition-smooth"
                  ...
                  <Button
                    type="button"
                    variant="outline"
                    className="bg-gradient-to-r from-blue-600 to-blue-700 text-white border-0 hover:from-blue-700 hover:to-blue-800 transition-smooth"
                    onClick={handleWalletAuth}
                    disabled={walletLoading}
                  >
                    <Wallet className="w-4 h-4 mr-2" />
                    {walletAddress ? (basename || 'Connected') : 'Wallet'}
                  </Button>
                </div>

                {walletAddress && (
                  <p className="text-xs text-center mt-2 text-muted-foreground">
                    {basename || `${walletAddress.substring(0, 6)}...${walletAddress.substring(walletAddress.length - 4)}`}
                  </p>
                )}
              </div>
